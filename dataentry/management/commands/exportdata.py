    ...
    def handle(self, *args, **kwargs):
        ...
        for app_config in apps.get_app_configs():
            try:
                ...
                pass
        if not model:
            self.stderr.write(self.style.ERROR(f'Model {model_name} could not found'))
            return
    
        # fetch data from model
        data = model.objects.all()

        # get timestamp from current data and time
        ...
        with open(file_path, 'w', newline='') as file:
            ...
            # we want to print names of the models that we are trying to export
